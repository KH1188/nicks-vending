  ...
  return (
    <header
      className={`fixed inset-x-0 top-0 z-50 transition-all duration-300 border-b
        ${scrolled ? 'bg-ink/95 backdrop-blur-sm border-ink-border' : 'bg-ink/70 backdrop-blur-sm border-transparent'}`}
    >
        ...
        <ul className="hidden md:flex items-center gap-1">
          {NAV_LINKS.map(({ label, href }) => (
            <li key={href}>
              <a
                href={href}
                className="px-4 py-2 text-sm font-medium text-white/70 rounded-lg
                  hover:text-white hover:bg-white/5 transition-colors duration-150
                  focus:outline-none focus:ring-2 focus:ring-neon-violet focus:ring-offset-1 focus:ring-offset-ink"
              >
                {label}
              </a>
            </li>
      ...
      <div
        ...
        } bg-ink border-b border-ink-border`}
      >
        <ul className="px-4 pb-4 pt-1 flex flex-col gap-1">
          {NAV_LINKS.map(({ label, href }) => (
            <li key={href}>
              <a href={href} onClick={close}
                className="block px-4 py-2.5 text-sm font-medium text-white/70 rounded-lg hover:text-white hover:bg-white/5 transition-colors">
                {label}
              </a>
            </li>
          ))}
